...
D3DFILLMODE             CurrentFillMd;//Способ отображения
...
HRESULT InitGeometry5() {
    ...
    for (i = NA * 48; i < NA * 96; i++) {
        ...
        pRVertices[i + NA * 48].nrm.y = t * sinf(D3DX_PI / 2) - pRVertices[i + NA * 48].nrm.z * sinf(D3DX_PI / 2);
...
void RenderScreen(void) {
    ...
    // ... продовження RenderScreen після setStreamSource ...
    p_d3d_Device->SetStreamSource(0, p_VertexBuffer, sizeof(MYVERTEX));
    p_d3d_Device->SetVertexShader(D3DFVF_CUSTOMVERTEX);

    // Малювання об'єкта (наприклад, кільця з InitGeometry5)
    p_d3d_Device->DrawPrimitive(D3DPT_TRIANGLELIST, 0, 3 * NA * 8 * 2);
...
//-----------------------------------------------------------------------------
// Name: MsgProc()
// Desc: Обробка повідомлень Windows
//-----------------------------------------------------------------------------
...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, INT) {
    ...
    _DELETE_(pRVertices);
    UnregisterClass("Grid3D_Class", wc.hInstance);
    return 0;
}
